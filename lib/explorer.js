...
var base_url = 'http://127.0.0.1:' + process.env.PORT + '/api/';
...
module.exports = {
    ...
    get_hashrate: function (cb) {
        ...
        request({
            uri: uri,
            json: true
        }, function (error, response, body) { //returned in mhash
            if (body.hashespersec) {
                return cb(body.hashespersec.toFixed(4));
            } else {
                return cb('-');
            }
        });
